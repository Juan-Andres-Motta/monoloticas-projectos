...
class AvroPulsarPublisher:
            ...
            try:
                ...
                for full_topic in topics_to_try:
                    try:
                        ...
                        print(
                            f"❌ Failed to create producer for {full_topic}: {topic_error}"
                        )
                        continue

                raise Exception(
                    f"Failed to create producer for any topic configuration: {topic}"
                )
    ...
    async def send_create_campaign_command(
        ...
        try:
            ...
            producer = await self._get_or_create_producer(
                PulsarTopics.CAMPAIGN_COMMANDS, schema, self._command_producers
            )
    ...
    async def send_add_video_url_command(
        self, campaign_id: UUID, video_url: str, correlation_id: Optional[str] = None
    ) -> str:
        ...
        try:
            ...
            # Get producer with Avro schema
            schema = AvroSchemaRegistry.get_command_schema(
    ...
    async def send_create_tracking_event_command(
        ...
        try:
            ...
            producer = await self._get_or_create_producer(
                PulsarTopics.TRACKING_COMMANDS, schema, self._command_producers
            )
    ...
    async def send_calculate_commission_command(
        ...
        try:
            ...
            producer = await self._get_or_create_producer(
                PulsarTopics.COMMISSION_COMMANDS, schema, self._command_producers
            )
    ...
    async def send_process_payment_command(
        ...
        try:
            ...
            producer = await self._get_or_create_producer(
                PulsarTopics.PAYMENT_COMMANDS, schema, self._command_producers
            )
    ...
    async def publish_domain_event(self, domain_event, service_name: str):
        ...
        try:
            ...
            if hasattr(record_class, "from_domain_event"):
                ...
                raise ValueError(
                    f"Record class {record_class} doesn't have from_domain_event method"
                )
            ...
            producer = await self._get_or_create_producer(
                event_topic, schema, self._event_producers
            )
