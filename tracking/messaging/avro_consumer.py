...
class AvroPulsarConsumer:
        ...
        try:
            ...
            for full_topic in topics_to_try:
                try:
                    ...
                    # Create consumer with Avro schema for tracking commands
                    # Use the appropriate Avro schema for this service
                    schema = self._get_consumer_schema()
    ...
    def _get_consumer_schema(self):
        """Get the appropriate Avro schema for this service's consumer"""
        schema_registry = AvroSchemaRegistry()

        # Return the primary command schema for this service
        if self.service_name == "tracking":
            return schema_registry.get_schema("tracking.create_event.command.v1")
        elif self.service_name == "campaign":
            return schema_registry.get_schema("campaign.create.command.v1")
        elif self.service_name == "commission":
            return schema_registry.get_schema("commission.calculate.command.v1")
        elif self.service_name == "payment":
            return schema_registry.get_schema("payment.process.command.v1")
        else:
            # Fallback to string schema if service type unknown
